...
export const calculateColumns = (windowWidth) => {
  ...
  export const splitCardsIntoMainAndLeftover = (cardData, columns) => {
    const mainGridCards = cardData.slice(0, Math.floor(cardData.length / columns) * columns);
    const leftoverCards = cardData.slice(Math.floor(cardData.length / columns) * columns);
